# MetaDefender Endpoint Security SDK Sample Code CPP for macOS
...
- Copy the files to a specific folder :
    ...
    - Header folder ".\inc" :
		...
		+ Copy the vmod folder to the ".\Inc" folder : located at "OESIS_V4_mac_4_3_xxxx_0r\inc\vmod"

	- License folder ".\license" :
		+ license.cfg
		+ pass_key.txt with pass_key provided
        
- Open Project with Xcode and config following items (if neeeded):
    - Build Settings:
        + Dynamic Library Install Name: set to Library folder (.\libs)
        + Dynamic Library Install Name Base: set to Library folder (.\libs)
        + Header Search Path: set to Headers folder (.\inc)
        + Library Search Path: set to Library folder (.\libs)
    -Build Phases:
        + Link Binary with Libraies: link with all .dylib in Library folder (.\libs)
        + Copy Files: copy all .dylib file in Library folder (.\libs) and  license.cfg and pass_key.txt in License folder (.\license)
    
- Build project in Xcode: go to Product then Build (Xcode -> Product -> Show Build Folder in Finder -> Products -> Debug).

## Usage

__sample-code-cpp COMMAND OPTIONS__
